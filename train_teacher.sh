...
# 
# 用法:
#   bash train_teacher.sh <exptid> [device] [num_envs] [mode] [bg]
# 
# 参数:
#   exptid   - 实验ID (必需)
...
#   bash train_teacher.sh test_cmg cuda:0 0 normal bg      # 后台运行 (SSH断开后继续)
#   bash train_teacher.sh test_cmg cuda:0 256 normal bg    # 256环境 + 后台运行
...
if [ -z "$exptid" ]; then
    echo "错误: 必须提供实验ID"
    echo "用法: bash train_teacher.sh <exptid> [device] [num_envs] [mode] [bg]"
    echo ""
    echo "示例:"
    echo "  bash train_teacher.sh test_cmg                      # 前台运行"
    echo "  bash train_teacher.sh test_cmg cuda:0 256 normal bg # 后台运行"
    exit 1
fi
